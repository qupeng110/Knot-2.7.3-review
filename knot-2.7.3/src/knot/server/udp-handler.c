...
static void udp_pktinfo_handle(const struct msghdr *rx, struct msghdr *tx)
{
	...
	/* Unset the ifindex to not bypass the routing tables. */ /* qp-nodes: 绕过路由表，将请求报文的目的地址变更为响应报文的源地址 */
...
int udp_master(dthread_t *thread)
{
	...
	unsigned cpu = dt_online_cpus(); /* qp-nodes: 获取可检测CPU个数，对于core则每个线程分配一个核心绑定 */
	if (cpu > 1) {
	...
	unsigned thr_id = dt_get_id(thread); /* qp-nodes: 根据线程ID获取本线程的控制数据块 */
	...
	knot_mm_t mm;
	mm_ctx_mempool(&mm, 16 * MM_DEFAULT_BLKSIZE);  /* qp-nodes: 预分配巨页 */
	...
	knot_layer_init(&udp.layer, &mm, process_query_layer()); /* qp-nodes: 回掉函数注册 */
	...
	for (;;) {
		...
		/* Check handler state. */ /* qp-nodes: 依次检查接口状态与运行状态 */
		...
		/* Cancellation point. */ /* qp-nodes: 当前任务线程被取消 */
		if (dt_is_cancelled(thread)) {
			break;
		}

		/* Wait for events. */ 
		int events = poll(fds, nfds, -1); /* qp-nodes: 阻塞等待报文到达 */
		if (events <= 0) {
			if (errno == EINTR) continue;
			break;
		}

		/* Process the events. */ /* qp-nodes：多接口轮询 等待报文 */
